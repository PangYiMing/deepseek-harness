...
The adapter follows [pi-ai 0.86.0](https://github.com/earendil-works/pi/blob/v0.86.0/packages/ai/CHANGELOG.md). `thinkingTokenBudgetField`, `vllmPriority`, and `supportsMaxOutputTokens` are opt-in gateway controls; `thinking.budget` joins the existing template placeholders. The SDK owns budget resolution and serialization. `supportsMidConvoEffort` and `allowedFallbackModels` remain catalog-owned because their correctness depends on exact Anthropic transports, model capabilities, and fallback pricing. 0.86.0's transcript mid-conversation switches — `supportsMidConvoSystemMessages` on the completions, Responses, Anthropic, and Mistral compat types, `supportsMidConvoToolAdditions` on completions, and `supportsMidConvoToolChanges` on Anthropic — join them as catalog-owned for the same reason, as does Anthropic `sessionAffinityFormat`. `deferredToolsMode` and `supportsToolReferences` are retired upstream and leave the gates; `mistral-conversations` gains a compat type whose single field is catalog-owned.
