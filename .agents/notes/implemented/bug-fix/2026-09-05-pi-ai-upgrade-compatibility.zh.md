...
适配器遵循 [pi-ai 0.86.0](https://github.com/earendil-works/pi/blob/v0.86.0/packages/ai/CHANGELOG.md)。`thinkingTokenBudgetField`、`vllmPriority` 和 `supportsMaxOutputTokens` 是显式启用的网关控制；`thinking.budget` 加入现有模板占位符。SDK 拥有预算解析和序列化。`supportsMidConvoEffort` 和 `allowedFallbackModels` 仍由目录拥有，因为其正确性依赖确切的 Anthropic 传输、模型能力和回退定价。0.86.0 的 transcript 中途对话开关——completions、Responses、Anthropic 和 Mistral compat 类型上的 `supportsMidConvoSystemMessages`、completions 上的 `supportsMidConvoToolAdditions`、Anthropic 上的 `supportsMidConvoToolChanges`——出于同样的原因加入目录拥有集合，Anthropic 的 `sessionAffinityFormat` 亦然。`deferredToolsMode` 和 `supportsToolReferences` 已在上游退役，离开门禁；`mistral-conversations` 获得 compat 类型，其唯一字段由目录拥有。
